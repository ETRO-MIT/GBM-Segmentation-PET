...
for fold in ['F1', 'F2', 'F3', 'F4', 'F5']:
    ...
    params = list(FET2TM.parameters()) + list(FET2SEG.parameters())
    
    loss_function = DiceCELoss(smooth_nr=1e-5, smooth_dr=1e-5, squared_pred=False, to_onehot_y=False, sigmoid=True)
    optimizer = torch.optim.SGD(params, lr, weight_decay=3e-5, momentum=0.99, nesterov=True)
    lr_scheduler = PolyLRScheduler(optimizer, lr, max_epochs)
    
    dice_metric_seg = DiceMetric(include_background=True, reduction="mean")
    dice_metric_batch_seg = DiceMetric(include_background=True, reduction="mean_batch")
    dice_metric_tm = DiceMetric(include_background=True, reduction="mean")
    dice_metric_batch_tm = DiceMetric(include_background=True, reduction="mean_batch")
    
    post_trans = Compose([Activations(sigmoid=True), AsDiscrete(threshold=0.5)])
    
    # DEFINE INFERENCE MODE
    def inference_TM(input):
        def _compute(input):
    ...
    for epoch in range(max_epochs):
        ...
        for batch_data in train_loader:
            ...
            with torch.cuda.amp.autocast():
                thres_pred = FET2TM(img)
                thres_pred = torch.unbind(thres_pred, dim=1)
    
                thres_in = thres_pred[0]
                #thres_in = post_trans(thres_in)
    
                inputs = torch.cat((img, thres_in), dim=1)
                seg_pred = FET2SEG(inputs)
                seg_pred = torch.unbind(seg_pred, dim=1)
                
                loss_TM = sum(0.5**i * loss_function(p, thres) for i, p in enumerate(thres_pred))
                loss_SEG = sum(0.5**i * loss_function(p, labels) for i, p in enumerate(seg_pred))
                loss = (alpha)*loss_TM + (1-alpha)*loss_SEG
                
            scaler.scale(loss).backward()
            scaler.step(optimizer)
            scaler.update()
            epoch_loss += loss.item()
            epoch_loss_TM += loss_TM.item()
        ...
        wandb.log({f"epoch_loss_TM_{fold}": epoch_loss_TM})
        wandb.log({f"epoch_loss_SEG_{fold}": epoch_loss_SEG})
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    thres_pred = inference_TM(val_img)
                    #thres_in = post_trans(thres_pred)
                    
                    val_inputs = torch.cat((val_img, thres_pred), dim=1)
                    seg_pred = inference_SEG(val_inputs)
                    
                    thres_pred = post_trans(thres_pred)
                    seg_pred = post_trans(seg_pred)
                    
                    dice_metric_seg(y_pred=seg_pred, y=val_labels)
                    dice_metric_batch_seg(y_pred=seg_pred, y=val_labels)
                    dice_metric_tm(y_pred=thres_pred, y=val_thres)
                    dice_metric_batch_tm(y_pred=thres_pred, y=val_thres)
                    
                    dice = dice_metric_seg(y_pred=seg_pred, y=val_labels)
                    nsd = compute_surface_dice(seg_pred, val_labels, class_thresholds=[1])
                    ave = abs(np.sum(seg_pred[0,0,:,:,:].detach().cpu().numpy()) - np.sum(val_labels[0,0,:,:,:].detach().cpu().numpy()))
                    ...
                    df.to_csv(f'results_ETE_2_{fold}.csv')
        
                    idx += 1
                
                metric_seg = dice_metric_seg.aggregate().item()
                metric_batch_seg = dice_metric_batch_seg.aggregate()
                dice_metric_seg.reset()
                dice_metric_batch_seg.reset()
                metric_tm = dice_metric_tm.aggregate().item()
                metric_batch_tm = dice_metric_batch_tm.aggregate()
                dice_metric_tm.reset()
                dice_metric_batch_tm.reset()
    
                wandb.log({f"dice_tm_{fold}": metric_tm})
                wandb.log({f"dice_seg_{fold}": metric_seg})
    
                if metric_seg > best_metric:
                    best_metric = metric_seg
                    ...
                    torch.save(
                        FET2TM.state_dict(),
                        os.path.join(model_root, f"best_FET2TM_ETE_2_{fold}.pth"),
                    )
                    torch.save(
                        FET2SEG.state_dict(),
                        os.path.join(model_root, f"best_FET2SEG_ETE_2_{fold}.pth"),
                    )
                    print("saved new best metric model")
                
                torch.save(
                    FET2TM.state_dict(),
                    os.path.join(model_root, f"latest_FET2TM_ETE_2_{fold}.pth"),
                )
                torch.save(
                    FET2SEG.state_dict(),
                    os.path.join(model_root, f"latest_FET2SEG_ETE_2_{fold}.pth"),
                )
