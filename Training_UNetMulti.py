...
def get_data(data_root, train_val_or_test):
  ...
  for subject in subjects[train_val_or_test]:
    ...
    dict['brain'] = os.path.join(f,'brain.nii.gz')
    dict['bkg'] = os.path.join(f,'bkg.nii.gz')
...
def get_dice(im1, im2, empty_score=0.0):
...
train_transforms = transforms.Compose([
    # DETERMINISTIC 
    transforms.LoadImaged(keys=["image", "label", "bkg", "brain"], ensure_channel_first=True),
    ConvertToBinaryLabelsd(keys=["label", "brain", "bkg"]),
    #transforms.ScaleIntensityRangePercentilesd(keys=["image"], lower=0, upper=100, b_min=0, b_max=1), ##
    transforms.NormalizeIntensityd(keys=["image"]),
    transforms.Orientationd(keys=["image", "label", "bkg", "brain"], axcodes="RAS"),
    transforms.Spacingd(keys=["image", "label", "bkg", "brain"], pixdim=(1.0, 1.0, 1.0), mode=("bilinear", "nearest", "nearest", "nearest")), 
    
    # STOCHASTIC 
    transforms.RandCropByPosNegLabeld(keys=["image", "label", "bkg", "brain"], spatial_size=size, label_key="label", num_samples=2),
    transforms.RandZoomd(keys=["image", "label", "bkg", "brain"], min_zoom=1, max_zoom=1.3, mode=("trilinear", "nearest", "nearest", "nearest")),
    transforms.RandFlipd(keys=["image", "label", "bkg", "brain"], prob=0.5, spatial_axis=0), #only allow lateral flipping (symmetrical axis)
    transforms.RandRotated(keys=["image", "label", "bkg", "brain"], range_x=0.15, range_y=0.15, range_z=0.15, prob=0.5)
    ])
...
for fold in ['F1', 'F2', 'F3', 'F4', 'F5']:
    ...
    for epoch in range(max_epochs):
        ...
        for batch_data in train_loader:
            ...
            inputs, tum, brain, bkg = (
                batch_data["image"].to(device),
                batch_data["label"].to(device),
                batch_data["brain"].to(device),
                batch_data["bkg"].to(device),
            )
            
            labels = torch.cat((tum, bkg, brain), dim=1)
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    val_inputs, val_tum, val_brain, val_bkg= (
                        val_data["image"].to(device),
                        val_data["label"].to(device),
                        val_data["brain"].to(device),
                        val_data["bkg"].to(device),
                    )
                    
                    val_labels = torch.cat((val_tum, val_bkg, val_brain), dim=1)
                    ...
                    dice = get_dice(val_outputs[0,0,:,:,:].detach().cpu().numpy(), val_labels[0,0,:,:,:].detach().cpu().numpy())
                    nsd = compute_surface_dice(val_outputs, val_labels, class_thresholds=[1,1,1], include_background=True)
                    ave = abs(np.sum(val_outputs[0,0,:,:,:].detach().cpu().numpy()) - np.sum(val_labels[0,0,:,:,:].detach().cpu().numpy()))
                    
                    row['Subject'] = val_subjects[idx]
                    row['Dice'] = dice
                    row['NSD'] = nsd[0,0].cpu().item()
                    row['AVE'] = ave
            
                    df = pd.concat([df, pd.DataFrame([row])], ignore_index=True)
                    df.to_csv(os.path.join(results_root, f'results_SegFet_multi_{fold}.csv'))
                ...
                metric_tum = metric_batch[0].item()
                metric_bkg = metric_batch[1].item()
                metric_brain = metric_batch[2].item()
                dice_metric.reset()
                dice_metric_batch.reset()
    
                wandb.log({f"dice_{fold}": metric, f"dice_tum_{fold}": metric_tum, f"dice_bkg_{fold}": metric_bkg, f"dice_brain_{fold}": metric_brain})
                ...
                if metric > best_metric:
                    ...
                    torch.save(
                        model.state_dict(),
                        os.path.join(model_root, f"SegFet_multi_best_{fold}.pth"),
                    )
                ...
                torch.save(
                    model.state_dict(),
                    os.path.join(model_root, f"SegFet_multi_latest_{fold}.pth"),
                )
