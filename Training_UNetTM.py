...
def get_data(data_root, train_val_or_test):
  ...
  for subject in subjects[train_val_or_test]:
    ...
    dict['label'] = os.path.join(f,'thres.nii.gz')
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    df.to_csv(os.path.join(results_root, f'results_SegThres_{fold}.csv'))
                ...
                if metric > best_metric:
                    ...
                    torch.save(
                        model.state_dict(),
                        os.path.join(model_root, f"SegThres_best_{fold}.pth"),
                    )
                ...
                torch.save(
                    model.state_dict(),
                    os.path.join(model_root, f"SegThres_latest_{fold}.pth"),
                )
